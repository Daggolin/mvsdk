...
void _UI_Refresh( int realtime )
{
	...
	if (Menu_Count() > 0) {
		...
		// draw cursor
		if (ui_widescreen.integer == 2 && mvapi >= 3 && uiInfo.uiDC.portraitMode && isMainMenu) { //Scale height on the main menu in portrait mode.
			UI_SetColor(NULL);
...
void _UI_MouseEvent( int dx, int dy )
{
	...
	if (ui_widescreen.integer == 2 && uiInfo.uiDC.portraitMode && isMainMenu)
